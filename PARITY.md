...
- [~] **GAP-112** Closed testers shouldn't have to manually configure a server on first launch. `DataPackageBootstrap.runIfNeeded()` (wired in `OmniTAKApp.onCreate()`) copies `assets/demo-package.zip` into the import dir on first run, then imports it — seeding `tak.engindearing.soy:8089:ssl` plus the bundled `engie-android.p12` cert + truststore. No UI tap required. Known caveat: shared identity (every tester connects as `engie-android`), fine for closed testing but visible as a single PPLI on the server until per-device cert enrollment via TAK's CSR port (8446) ships. **Shipped on Android (v0.3.0 vc29).** iOS parity pending.
